...
def preprocess_eeg_file(edf_path: Path, plots_dir: Path):
   ...
   # The function can return a list of figures, so we loop through it
    ...
    print("Saved before/after bad channel plots.")
     ...
     # 5. Create and reject epochs
    print("Segmenting data and rejecting bad epochs...")
    epochs = mne.make_fixed_length_epochs(raw_after_ica, duration=5, overlap=1, preload=True)
    fig_drop_log = epochs.plot_drop_log(show=False)
    drop_log_path = plots_dir / f"{base_filename}_drop_log.png"
    fig_drop_log.savefig(drop_log_path, dpi=150)
    plt.close(fig_drop_log)
    print(f"Saved epoch drop log to: {drop_log_path}")

    reject_criteria = dict(eeg=150e-6) # 150 µV
    original_n_epochs = len(epochs)
    epochs.drop_bad(reject=reject_criteria)
    print(f"Dropped {original_n_epochs - len(epochs)} epochs out of {original_n_epochs}.")
    
    # 6. Final PSD topography plot
    # ✅ NEW: Safety check to prevent crash if all epochs were dropped
    if len(epochs) == 0:
        print("⚠️ Warning: All epochs were dropped! Skipping final PSD plot.")
    else:
...
if __name__ == "__main__":
    ...
    out_dir = base_dir / "Data" / "processed data" / "XU_Processed_Manual_Awake"
    plots_dir = out_dir / "plots"
    out_dir.mkdir(parents=True, exist_ok=True)
    plots_dir.mkdir(parents=True, exist_ok=True)

    file_names = ["XUAWAKE7", "XUAWAKE60", "XUAWAKET100"]
    ...
    for name in file_names:
        ...
        print(f"Saved cleaned epochs to: {out_fif}")
